...
class SchemeViewTest(TestCase):
    ...
    def test_return_404_not_found(self):
        resp_get = self.client.get('/core_api/scheme/55/', HTTP_AUTHORIZATION=f'Token {self.user_1.auth_token}')
        resp_delete = self.client.delete('/core_api/scheme/55/', HTTP_AUTHORIZATION=f'Token {self.user_1.auth_token}')
        resp_update = self.client.put('core_api/scheme/55/', HTTP_AUTHORIZATION=f'Token {self.user_1.auth_token}')
