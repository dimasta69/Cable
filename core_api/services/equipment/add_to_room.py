...
class AddEquipmentFromRoomService(PresenceChecksMixin, ResourceAccessMixin, ServiceWithResult):
    room_id = forms.IntegerField(required=True)
    equipment_id = forms.IntegerField(required=True)
    current_user = ModelField(User)

    custom_validations = ["run_presence_checks", "equipment_room_null", "access_presence"]
    presence_checks = [
